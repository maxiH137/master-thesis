...
models = ['deepconvlstm', 'tinyhar'] # 0
training_status = [True, False] # 1
numbers = [100] # 2
datasets = ['wear', 'wetlab'] # 3
sampling = ['shuffle', 'unbalanced', 'balanced', 'sequential'] # 4
local_update = [2, 5] # 5
user_range = [2, 5] # 6
noise = [0.0,0.1] # 7
clipping = [0.0, 0.5, 1.0, 1.5] # 8

# Generate all combinations
combinations = list(product(models, training_status, numbers, datasets, sampling, local_update, user_range, noise, clipping))
used_comb = []
# Print combinations
...
class Evaluation():
    ...
    def calculate_accuracy_metrics(self, parser_args):
        ...
        for combination in combinations:
            if (combination[0] == self.args['model'] 
                and combination[1] == self.args['trained'] 
                and combination[2] == self.args['datapoints'] 
                and combination[3] == self.args['dataset'] 
                and combination[4] == self.args['sampling'] 
                and combination[5] == self.args['num_local_updates'] 
                and combination[6] == self.args['user_range'] 
                and combination[7] == self.args['noise'] 
                and combination[8] == self.args['clipping']):
        ...
        if correct_combination in used_comb:
            print(f"Comb {run_id} already used.")

        used_comb.append(correct_combination)
        ...
        with open('output_multFix.txt', 'a') as f:
            sys.stdout = f
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_strat_array', nargs='+', default=['llbgSGD','llbgAVG', 'iRLG', 'gcd', 'random'], type=str)
   # parser.add_argument('--runs', nargs='+', default=['TAL-253', 'TAL-258', 'TAL-264', 'TAL-274'], type=str) # DeepConv, SGD, Untrained, WEAR
    ...
    args.runs = [f'TAL-{i}' for i in range(1833, 1960)]
    args.runs += [f'TAL-{i}' for i in range(1961, 2024)]
    if 'TAL-1445' in args.runs:
        args.runs.remove('TAL-1445')
    if 'TAL-1446' in args.runs:
        args.runs.remove('TAL-1446')
    if 'TAL-1447' in args.runs:
        args.runs.remove('TAL-1447')
