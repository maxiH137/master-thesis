...
def group_and_average(runs, filter_terms):
    ...
    try:
        ...
        for run, data in runs.items():
            ...
            if len(split) >= 2:
                if(len(filter_terms) > 3):
                    last_part = [split[3] , *split[-3:]]  # Take only the last 4 values
                else:
                    last_part = split[-3:]
...
# Main execution
output_file = "output_local.txt"
output_dir = "TAL_Run_Plots_local"
avg_output_dir = "TAL_Avg_Plots_local"
data_output_dir = "DATA_CSV_local"
