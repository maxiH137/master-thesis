...
local_update = [2, 5] # 5
noise = [0,0.1] # 6
clipping = [0, 0.5, 1.0, 1.5] # 7

# Generate all combinations
combinations = list(product(models, training_status, numbers, datasets, sampling, local_update, noise, clipping))
used_combinations = []
# Print combinations
    ...
    def calculate_accuracy_metrics(self, parser_args):
        ...
        for combination in combinations:
            if (combination[0] == self.args['model'] 
                ...
                and combination[6] == self.args['noise'] 
                and combination[7] == self.args['clipping']):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_strat_array', nargs='+', default=['llbgAVG', 'bias-corrected', 'iRLG', 'gcd', 'ebi', 'wainakh-simple', 'wainakh-whitebox'], type=str)
   ...
   # args.runs = [f'TAL-{i}' for i in range(606, 669)] # Local without DP
    args.runs = [f'TAL-{i}' for i in range(606, 941)] # Local with DP
