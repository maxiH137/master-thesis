...
class Leakage():
    ...
    def main(self, args):
        ...
        if args.neptune:
            ...
            run['args/user_range'] = args.user_range
        ...
        lnAcc_all = 0
        subject_lists = []
        if(config['dataset_name'] == 'wear'):
            subjects_list_high = [17,9,0,13,16]
            subjects_list_middle = [11,5,6,4,12]
            subjects_list_low = [14,1,7,1,2]
        else:
            subjects_list_high = [6,2,18,5,19]
        ...
        for subject_multi in range(3):
            for label_strat in strat_array:
                for i, anno_split in enumerate(config['anno_json']):
                    ...
                    if(args.avg == 'multiU'):
                        cfg_case['user']['user_range'] = [0, args.user_range]
                    ####################################################
                    ...
                    users = breaching.cases.construct_user(model, loss_fn, cfg_case, setup, train_dataset, subject_lists[subject_multi])
                    ...
                    for idx, (shared_data, payload, true_user_data) in enumerate(zip(shared_user_data, payloads, true_user_data_all)):
                        shared_data['gradients'] = [grad * 10000 for grad in shared_data['gradients']]
