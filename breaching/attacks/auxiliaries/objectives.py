...
class GradientLoss(torch.nn.Module):
    ...
    def _grad_fn_multi_step(self, model, candidate, labels):
        ...
        for i in range(self.local_hyperparams["steps"]):
            ...
            # labels = self.local_hyperparams["labels"][i] # Changed MH2025 to avoid exception 
            labels_for_loss = labels[seen_data_idx : seen_data_idx + self.local_hyperparams["data_per_step"]] # Changed MH2025 to avoid exception 
            with torch.autocast(candidate.device.type, enabled=self.cfg_impl.mixed_precision):
                task_loss = self.loss_fn(func_model(params, buffers, data), labels_for_loss)
