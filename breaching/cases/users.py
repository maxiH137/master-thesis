...
def construct_user(model, loss_fn, cfg_case, setup, dataset, subjects=None):
    """Interface function."""
    if cfg_case.user.user_type == "local_gradient":
        dataloader = construct_dataloader(cfg_case.data, cfg_case.impl, user_idx=cfg_case.user.user_idx, dataset=dataset)
        ...
        #random_numbers = torch.randint(int(min(dataset.ids)), int(max(dataset.ids)), (cfg_case.user.user_range[1],))
        
        for idx in range(*cfg_case.user.user_range):
            dataset_copy = copy.deepcopy(dataset)

            float_idx = float(subjects[idx])
            dataset_copy.labels = dataset.labels[dataset.ids == float_idx]
            dataset_copy.features = dataset.features[dataset.ids == float_idx]
            dataset_copy.ids = dataset.ids[dataset.ids == float_idx]
            
            dataloaders += [construct_dataloader(cfg_case.data, cfg_case.impl, user_idx=idx, dataset=dataset_copy)]
            indices += [idx]
...
class UserSingleStep(torch.nn.Module):
    ...
    def _load_data(self, setup=None):
        ...
        for idx, data_block in enumerate(self.dataloader):
            if self.old_block != []:
                if any(torch.equal(data_block['labels'], old['labels']) for old in self.old_block):
                    continue
            
            data_blocks += [data_block]
            self.old_block += [data_block]
            try:
                num_samples += data_block["labels"].shape[0]
            except:
                num_samples += data_block[0].shape[0]
            if num_samples > self.num_data_points:
                break

         #Commented out to load whole dataset for HAR Sensing evaluation
        if num_samples < self.num_data_points:
            raise ValueError(
    ...
    def compute_local_updates(self, server_payload):
        ...
        with torch.no_grad():
            ...
            if buffers is not None:
                ...
                self.model.train()
        log.info(
            f"Computing user update on user {self.user_idx} in model mode: {'training' if self.model.training else 'training'}."
        )
        ...
        for step in range(self.num_local_updates):
            ...
            input_data = data["inputs"].unsqueeze(1) # Change to inputs, add dimension for channel MH_2025
            state_dict_train = self.model.state_dict()
            outputs = self.model(input_data)
            loss = self.loss(outputs, data["labels"])
            loss.backward()

            grads_ref = [p.grad for p in self.model.parameters()]
            grad_list.append(grads_ref)
            if self.clip_value > 0:
