...
def collate_function(batch):
    ...
    def _stack_tensor(tensor_list):
        ...
        tensor_list = [torch.tensor(t) if isinstance(t, np.ndarray) else t for t in tensor_list]
...
def _split_dataset_har(dataset, cfg_data, user_idx=None, return_full_dataset=False):
    if not return_full_dataset:
        ...
        if cfg_data.partition == "balanced":
            try:
                ...
                data_per_class_per_user = len(dataset) // dataset.classes // cfg_data.default_clients
            if data_per_class_per_user < 1:
                raise ValueError("Too many clients for a balanced dataset.")
            data_ids = []
            for class_idx in range(dataset.classes):
