...
class HonestServer:
    ...
    def run_protocol(self, user, multiUser=False):
        ...
        for query_id in self.queries():
            ...
            if multiUser:
                shared_data_per_round, true_user_data_per_round = user.compute_local_updates(server_payload, True)
            else:
                shared_data_per_round, true_user_data_per_round = user.compute_local_updates(server_payload)
            # true_data can only be used for analysis
            payloads += [server_payload]
            shared_user_data += [shared_data_per_round]
            true_user_data += [true_user_data_per_round]
