...
def report(
    ...
    elif (metadata["modality"] == "inertial"):
        modality_metrics = _run_har_metrics(
            reconstructed_user_data,
            true_user_data,
            server_payload,
            model,
    ...
    for idx, payload in enumerate(server_payload):
        ...
        with torch.no_grad():
            ...
            true_data = true_user_data["data"].unsqueeze(1).to(device=setup["device"])
            feats_true = model(true_data)
...
def _run_vision_metrics(
    ...
    ds = ds.squeeze(0) # Squeeze first dimension
    dm = dm.squeeze(0) # Squeeze dimension again
