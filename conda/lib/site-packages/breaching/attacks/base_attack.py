    ...
    def _recover_label_information(self, user_data, server_payload, rec_models):
        ...
        if self.cfg.label_strategy is None:
            ...
            for y in range(num_classes):
                first = average_bias[average_bias.sort(descending=True).indices[y]] 
                m_impactListTmp = []
                gcdTmp = []
                float_list_impact = []
                for x in range(num_classes):
                    second = average_bias[average_bias.sort(descending=True).indices[x]]
                    gcd = int(abs(round((first - second).item(), 2)) * 100)
                    if (gcd != 0):
                        gcdTmp.append(round((first - second).item(), 4))
                        m_impactListTmp.append(gcd)
                        float_list_impact.append(abs((first - second).item()))
            ...
            candidates = [gcd_avg if candidate == average_bias_correct_label else candidate for candidate in candidates]
            

            if (checkCandidate):
                m_impact = -gcd
            else:
                m_impact = average_bias_correct_label
