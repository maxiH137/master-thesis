    ...
    def _recover_label_information(self, user_data, server_payload, rec_models):
        ...
        if user_data[0]["metadata"]["local_hyperparams"] is not None:
            for shared_data in user_data:
                shared_data["gradients"] = [g * -1 for g in shared_data["gradients"]]
        ...
        if self.cfg.label_strategy is None:
            ...
            if self.cfg.label_strategy == "wainakh-simple":
                ...
                for query_id, shared_data in enumerate(user_data):
                    ...
                    #g_i = torch.where(shared_data["gradients"][-2] < 0, shared_data["gradients"][-2], torch.zeros_like(shared_data["gradients"][-2])).sum(dim=1)
                    m_query = (
                ...
                for query_id, model in enumerate(rec_models):
                    ...
                    for class_idx in range(num_classes):
                        ...
                        fake_labels = torch.as_tensor([class_idx] * num_data_points, dtype=int)
            ...
            labels = torch.stack(label_list)
        elif self.cfg.label_strategy == "gcd": 
            bias_per_query = [shared_data["gradients"][-1] for shared_data in user_data]
            label_list = []
            # Stage 1
            average_bias = torch.stack(bias_per_query).mean(dim=0)
            ...
            gcdTmp = [] 
            gcdListN = []
            float_list_impact_all = []


            for y in range(num_classes):
                first = average_bias[average_bias.sort(descending=True).indices[y]].item()
                m_impactListTmp = []
                gcdTmp = []
                float_list_impact = []
                for x in range(num_classes):
                    second = average_bias[average_bias.sort(descending=True).indices[x]].item()
                    distance = first - second
                    gcd = int(abs(round(distance, 2)) * 100)
                    if (gcd != 0):
                        gcdTmp.append(round(distance, 4))
                        m_impactListTmp.append(gcd)
                        float_list_impact.append(distance)

                gcd = math.gcd(*m_impactListTmp) / 100
                m_impactList.append(m_impactListTmp)
                float_list_impact_all.append(float_list_impact) 
                gcdList.append(gcd)
                gcdListN.append(gcdTmp)
                
            # checkCandidate = all(math.isclose(value % gcd_candidate, 0, abs_tol=1e-2) for value in average_bias)
            score = float('inf')
            gcd = 0
            
            candidates = []
            for clist in float_list_impact_all:
                for candidate in clist:
                    score_candidate = sum(average_bias % candidate)
                    if score_candidate < score:
                        score = score_candidate
                        gcd = candidate
                candidates.append(-gcd)

            gcd_avg = 0
            checkCounter = 0    
            for candidate in candidates:
                checkCandidate = all(math.isclose(value % candidate, 0, abs_tol=1e-2) for value in average_bias)
                if(not checkCandidate):
                    candidates[candidates.index(candidate)] = average_bias_correct_label
                else:
                    gcd_avg += candidate
                    checkCounter += 1
            if checkCounter != 0:
                gcd_avg = gcd_avg / checkCounter

            #m_impact = -max(abs(candidate) for candidate in candidates)
            candidates = [m_impact if candidate > 0 else candidate for candidate in candidates]
            ...
            while len(label_list) < num_data_points:
                ...
                label_list.append(selected_idx)
                #average_bias[selected_idx] -= m_impact
                average_bias[selected_idx] -= candidates[selected_idx]
            labels = torch.stack(label_list)
        ...
        elif self.cfg.label_strategy == "llbgSGD":  
            # our attack using bias gradient and theoretical value for impact
            v_hat = self.cfg.approx_avg_conf
        ...
        elif self.cfg.label_strategy == "llbgAVG":  
            # our attack using bias gradient and theoretical value for impact
            v_hat = self.cfg.approx_avg_conf
            self.conf_stats = {i: v_hat for i in range(1000)}
            ...
            if user_data[0]["metadata"]["local_hyperparams"] is not None:
                ...
                #data_per_batch = user_data[0]["metadata"]["local_hyperparams"]["data_per_step"]
                num_steps = user_data[0]["metadata"]["local_hyperparams"]["steps"]
                if "num_users" in user_data[0]["metadata"]:
                    num_users = user_data[0]["metadata"]["num_users"]
            m_impact = - 1 / data_per_batch
            user_data[0]["metadata"]["local_hyperparams"] = None
            
            # Impact of original LLBG was too high for DeepConvLSTM and TinyHAR
            # Used impact estimation of LLG instead
            g_i = torch.stack(bias_per_query).sum(dim=0)
            m_query = (
                        torch.where(g_i < 0, g_i, torch.zeros_like(g_i)).sum() * (1 + 1 / num_classes) / num_data_points
                    )
            s_offset = 0
            ...
            while len(label_list) < num_data_points:
                selected_idx = average_bias.argmin()
                #selected_idx = torch.tensor(valid_classes[average_bias[valid_classes].argmin()].item())
                model_conf = self.conf_stats[selected_idx.item()]
                label_list.append(selected_idx)
                average_bias[selected_idx] -= m_impact * (1 - model_conf)
            labels = torch.stack(label_list)

        elif self.cfg.label_strategy == "iLRG": # adapted from: https://github.com/BUAA-CST/iLRG/blob/master/methods.py
            probs = [torch.ones(num_classes) / num_classes for _ in  range(num_classes)]
            ...
            coefs.append([1 for _ in range(num_classes)])
            values.append(num_data_points)
            ...
            for i in range(num_classes):
                ...
                coefs.append(coef)
                values.append(num_data_points * bias_per_query[0][i])
            ...
            res = np.where(res <= num_data_points, res, 0)
            err = res - res_float
            num_mod = np.sum(res) - num_data_points
            if num_mod > 0:
                inds = np.argsort(-err)
        ...
        elif len(labels) > num_data_points:
            labels = labels[:num_data_points]
