...
def main(args):
    ...
    for i, anno_split in enumerate(config['anno_json']):
        ...
        for _, (inputs, targets) in enumerate(val_loader, 0):
            ...
            OriginalImg = onedimdata.squeeze(1).reshape(5000, config['dataset']['input_dim'])
            ReconstructedImg = reconstructed_user_data.squeeze(1).reshape(5000, config['dataset']['input_dim'])
        ...
        if trained:
            run[split_name + "/data/labelT-csv"].upload('labelsT_' + str(args.model) + '.csv')
            run[split_name + "/data/gradientT-csv"].upload('gradientsT_' + str(args.model) + '.csv')
        else:
            run[split_name + "/data/label-csv"].upload('labels_' + str(args.model) + '.csv') 
            run[split_name + "/data/gradient-csv"].upload('gradients_' + str(args.model) + '.csv') 
